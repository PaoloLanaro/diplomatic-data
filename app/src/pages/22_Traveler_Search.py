...
if st.button('Search for articles',
    ...
    if len(st.session_state['search_terms']) != 0:
        articles = requests.post('http://api:4000/article/articles', json=st.session_state['search_terms'])
...
if 'articles' in st.session_state:
    ...
    if st.session_state['num_articles'] != 0: 
        title_request = requests.get(f'http://api:4000/article/title/{articles_json[article_idx]["url"]}')
